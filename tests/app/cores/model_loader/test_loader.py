...
def mock_dependencies():
	with (
		patch('app.cores.model_loader.model_loader.SessionLocal') as mock_session,
...
def test_move_to_device_logs_to_empty_success(caplog: pytest.LogCaptureFixture):
	# Arrange
	mock_pipe = MagicMock()
	device = 'cpu'
	log_prefix = 'Test model'
	...
	# Assert
	assert any('moved to cpu device using to_empty()' in record.getMessage() for record in caplog.records)


def test_move_to_device_logs_fallback_attribute_error(caplog: pytest.LogCaptureFixture):
	# Arrange
	mock_pipe = MagicMock()
	device = 'cpu'
	log_prefix = 'Test model'
	...
	# Assert
	assert any('moved to cpu device using to()' in record.getMessage() for record in caplog.records)


def test_move_to_device_logs_fallback_type_error(caplog: pytest.LogCaptureFixture):
	# Arrange
	mock_pipe = MagicMock()
	device = 'cpu'
	log_prefix = 'Test model'
